...
with st.sidebar:
    ...
    st.markdown(f"**Background:** The recurrent neural network (RNN) model is trained on [ASOP documents]({link2}) for educational purposes.") 

    st.subheader('What is RNN?')
    st.write("RNN is an artificial neural network designed to recognize patterns in sequences of data, such as text. The model is trained at a character unit.")
    st.write("The model inferences the next character from the 40-character context window. The words and structure of sentences are generated by shifting through the context window one by one.")
    
    st.image('images/RNN_structure.png', caption="The RNN structure used in this example")
    st.write("The current RNN structure uses Long Short-Term Memory (LSTM) layers to create a memory of previous inputs in its internal state. Dropout layers are used to prevent overfitting.")
    
    with st.container(border=True):
        st.subheader('⚙️ Parameters')
        p_temp = st.slider('Temperature:', min_value=0.25, max_value=2.0, value=1.0, step=0.25, help="Higher temperature leads to more randomness. See chart below.")
        p_max = st.slider('ASOP character length:', min_value=200, max_value=500, value=400, step=100)    
    
        df = pdf_transform(my_array, temperature=1.0, column_name="Base PDF")
        df["Temp: "+str(p_temp)] = pdf_transform(my_array, temperature=p_temp, column_name="Temp: "+str(p_temp))
    
    st.caption("The following chart illustrates what temperature does to the multinomial probability distribution predicting the next character. The actual PDF differs for each inference and has 69 possible cases (26 lower cases + numbers + special characters).")
    melted_df = df.reset_index().melt(id_vars='index', var_name='ParameterSet', value_name='Probability')
    ...
    st.write(f"The Python code and further detailed documentation of the project are in [GitHub]({link1}).")
